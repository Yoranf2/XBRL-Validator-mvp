#!/usr/bin/env python3
"""
Debug script to investigate eba_met:* concept resolution issues
"""
import sys
...
def test_concept_resolution():
    print("Testing eba_met:* concept resolution...")
    ...
    if sample_file.exists():
        ...
        try:
            file_source = FileSource.openFileSource(str(sample_file), cntlr)
            ...
            if model_xbrl:
                ...
                # Check namespaces in model
                print("\nChecking loaded namespaces...")
                namespaces = set()
                for qname in model_xbrl.qnameConcepts.keys():
                    namespaces.add(qname.namespaceURI)
                
                eba_namespaces = [ns for ns in namespaces if 'eba.europa.eu' in ns]
                print(f"Found {len(eba_namespaces)} EBA namespaces:")
                for ns in sorted(eba_namespaces):
                    print(f"  - {ns}")
    ...
    # Test loading with explicit met.xsd if missing
    print("\n" + "="*60)
    print("Testing with explicit met.xsd inclusion...")
    
    # Try loading met.xsd directly
    met_xsd_path = PROJECT_ROOT / "backend" / "github_work" / "taxonomies" / "eba" / "rf40" / "dict-unpacked" / "www.eba.europa.eu" / "eu" / "fr" / "xbrl" / "crr" / "dict" / "met" / "met.xsd"
    
    if met_xsd_path.exists():
        print(f"✓ Found met.xsd at: {met_xsd_path}")
        try:
            # Load met.xsd directly into model manager
            met_model = mm.load(str(met_xsd_path))
            if met_model:
                print("✓ Successfully loaded met.xsd directly")
                
                # Check if concepts are now available in the loaded model
                eba_met_concepts_after = []
                if hasattr(met_model, 'qnameConcepts'):
                    for qname, concept in met_model.qnameConcepts.items():
                        if qname.namespaceURI == "http://www.eba.europa.eu/xbrl/crr/dict/met":
                            eba_met_concepts_after.append(concept)
                
                print(f"After loading met.xsd: Found {len(eba_met_concepts_after)} eba_met concepts")
                if eba_met_concepts_after:
                    print("Sample concepts:")
                    for concept in eba_met_concepts_after[:5]:
                        print(f"  - {concept.qname}")
            else:
                print("✗ Failed to load met.xsd directly")
        except Exception as e:
            print(f"Error loading met.xsd directly: {e}")
    else:
        print(f"✗ met.xsd not found at expected path: {met_xsd_path}")
